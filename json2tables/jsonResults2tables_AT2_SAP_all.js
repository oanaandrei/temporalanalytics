...
if (process.argv.length<7) {
  console.log('Usage: node jsonResults2tables_AT2_SAP_all.js results_Pattern_PAM_K2.json K [latex|csv] [level1Dict|UseStop|level2Dict|level3Dict|threelevels|others] [VPVCSCinit|VP1|VC1|SC1|VP2|SC2]');
  // VPSVCSCinit only work for latex
  // VP1, VC1, and SC1 only work for csv
  // for VP2 and SC2 input also the time interval; use only with the dictionary options threelevels
  console.log('Usage: node jsonResults2tables_AT2_SAP_all.js results_Pattern_PAM_K2.json K csv threelevels [VP2|SC2] start end');
  return;
}
...
fs.readFile(process.argv[2], function(err, data) {
  ...
  if (propType.indexOf("SC2") == 0 || propType.indexOf("VP2") == 0) {
  	if (process.argv.length<9) {
  		console.log('Usage: node jsonResults2tables_AT2_SAP_all.js results_Pattern_PAM_K2.json K latex threelevels [VP2|SC2] start end');
  		return;
  	}
  ...
  else if (dictType.indexOf("others") == 0) {
  	dict = dict_others;
  }  
  else {
  ...
  var tableHeader_SAP_VPVCSCinit = "\\begin{table}\n\\center\n\\caption{Properties \\VisitProbInit, \\VisitCountInit, and \\StepCountInit\\ for PAM(" + K + ")}\\label{table:AT2_SAP_VP1_VC1_SC1_K" + K + "_" + dictType + "}\n{\\footnotesize\n\\begin{tabular}{cc";
  
  var table_headerSAP_VP2 = "\\begin{table}\n\\center\n\\caption{Property \\VisitProbBetween\\ for PAM(" + K + ") and time interval [" + timecut_start + ", " + timecut_end + "]}\\label{table:AT2_SAP_VPbtw_K" + K + "_" + timecut_start + "_" + timecut_end + "}\n{\\footnotesize\n\\begin{tabular}{|c|";

  var table_headerSAP_SC2 = "\\begin{table}\n\\center\n\\caption{Property \\StepCountBetween\\ for PAM(" + K + ") and time interval [" + timecut_start + ", " + timecut_end + "]}\\label{table:AT2_SAP_SCbtw_K" + K + "_" + timecut_start + "_" + timecut_end + "}\n{\\footnotesize\n\\begin{tabular}{|c|";
  ...
  var strSAP_VPVCSCinit = tableHeader_SAP_VPVCSCinit + table_header;
...
// Printing results into a latex table
  ...
  strSAP_VPVCSCinit += "\\VisitProbInit";

// Property SAP_VP1
  for (var l=0; l<nTimecuts; l++){
	for (var i=0; i<nStates*K; i++) Kcolouring[i] = 0;
	strSAP_VPVCSCinit += " & [" + timecuts[l].start + "," + timecuts[l].end + "]";
  	...
  	for(var s=0; s<nStates*K-K+1; s=s+K){
  		...
  		for (var x=s; x<s+K; x++) {
  			strSAP_VPVCSCinit += " & " + orderingColourHighBest(Kcolouring[x],K) + tableSAP_VP1[l][x] + "}"; 
  		}  		
  	}
//		// no colouring 	
...
//  	}
  ...
  strSAP_VPVCSCinit += "\\midrule \n";	

  var csvSAP_VP1 = csv_header_init;
  for (var l=0; l<nTimecuts; l++) {
  	csvSAP_VP1 += l + "," + "[" + timecuts[l].start + "," + timecuts[l].end + "]" + "," + tableSAP_VP1[l].join(",") + "\n";
  }  
...
// Property SAP_VP2
  ...
  strSAP_VP2 += "\\end{tabular}\n}\n\\end{table}";	



  var csvSAP_VP2 = csv_header_btw;
  for (var l=0; l<nStates; l++) {
  	csvSAP_VP2 += l + "," + dict[l].shortLabel + "," + tableSAP_VP2[l].join(",") + "\n";
  }  


 // Property SAP_VC1   
  strSAP_VPVCSCinit += "\\VisitCountInit"; 
  for (var l=0; l<nTimecuts; l++){
    for (var i=0; i<nStates*K; i++) 
    	Kcolouring[i] = 0;
	strSAP_VPVCSCinit += " & [" + timecuts[l].start + "," + timecuts[l].end + "]";
  	...
  	for(var s=0; s<nStates*K-K+1; s=s+K){
  		...
  		for (var x=s; x<s+K; x++) {
  			strSAP_VPVCSCinit += " & " + orderingColourHighBest(Kcolouring[x],K) + tableSAP_VC1[l][x] + "}"; 
  		}  		
  	}
  	strSAP_VPVCSCinit += "\\\\ \n";
  }
  ...
  var csvSAP_VC1 = csv_header_init;
  for (var l=0; l<nTimecuts; l++) {
  	csvSAP_VC1 += l + "," + "[" + timecuts[l].start + "," + timecuts[l].end + "]" + "," + tableSAP_VC1[l].join(",") + "\n";
  }  
...
// Property SAP_SC1
  strSAP_VPVCSCinit += "\\StepCountInit";  
  for (var l=0; l<nTimecuts; l++){
	for (var i=0; i<nStates*K; i++) Kcolouring[i] = 0;
	strSAP_VPVCSCinit += " & [" + timecuts[l].start + "," + timecuts[l].end + "]" ;
	...
	// colouring
  	for(var s=0; s<nStates*K-K+1; s=s+K){
  		...
  		for (var x=s; x<s+K; x++) {
  			strSAP_VPVCSCinit += " & " + orderingColourLowBest(Kcolouring[x],K) + tableSAP_SC1[l][x] + "}"; 
  		}  		
  	}
  	strSAP_VPVCSCinit += "\\\\  \n";
  }
  strSAP_VPVCSCinit += "\\bottomrule \\\\ \n \\end{tabular}}\n\\end{table}";	

  var csvSAP_SC1 = csv_header_init;
  for (var l=0; l<nTimecuts; l++) {
  	csvSAP_SC1 += l + "," + "[" + timecuts[l].start + "," + timecuts[l].end + "]" + "," + tableSAP_SC1[l].join(",") + "\n";
  }  
...
// Property SAP_SC2
  for (var l=0; l<nStates; l++){
	...
	// colouring
	// old: UseStop position is hard coded: the last -K from the upper bound is due to un-needed UseStop on the last column
  	for(var s=0; s<nStates*K-K+1; s=s+K){
  		for (var x=s; x<s+K-1; x++) {
  			for (var y=x+1; y<s+K; y++) {
  ...
  strSAP_SC2 += "\\end{tabular}}\n\n\\end{table}";	

  var csvSAP_SC2 = csv_header_btw;
  for (var l=0; l<nStates; l++) {
  	csvSAP_SC2 += l + "," + dict[l].shortLabel + "," + tableSAP_SC2[l].join(",") + "\n";
  }  

  
  if (fileType.indexOf("latex") == 0) {
  	if (propType.indexOf("VPVCSCinit") == 0) 
  		console.log(strSAP_VPVCSCinit + "\n\n");
  	...
  	if (propType.indexOf("VP2") == 0) console.log(csvSAP_VP2);
  	if (propType.indexOf("SC1") == 0) console.log(csvSAP_SC1);
  	if (propType.indexOf("SC2") == 0) console.log(csvSAP_SC2);

...
if (process.argv.length<7) {
  console.log('Usage: node jsonResults2tables_AT1.js results_Pattern_PAM_K2.json K [latex|csv] [4statesL1|UseStop|8statesL1|twolevels|4statesL2|threelevels|alt|full] [VP1|SC1|VC1|VP2|SC2]');
  // for VP2 and SC2 input also the time interval; only works with the dictionary options [twolevels|threelevels]
  console.log('Usage: node jsonResults2tables_AT1.js results_Pattern_PAM_K2.json K latex [twolevels|threelevels] [VP2|SC2] start end');
  return;
}
...
fs.readFile(process.argv[2], function(err, data) {
  ...
  if (propType.indexOf("SC2") == 0 || propType.indexOf("VP2") == 0) {
  	if (process.argv.length<9) {
  		console.log('Usage: node jsonResults2tables_AT1.js results_Pattern_PAM_K2.json K latex twolevels [VP2|SC2] start end');
  		return;
  	}
...
//	{ id: "9", label: "ATStatsView", shortLabel: "Stats" },
 ...
 // 	{ id: "10", label: "ATUsageBarChartATOverallUsageView", shortLabel: "UBCOverallUsage" },
  ...
  else if (dictType.indexOf("alt") == 0) {
  	dict = dict_alt;
  }  
  ...
  // Property SAP_VP1
  var table_headerSAP_VP1 = "\\begin{table}\n\\center\n\\caption{Property SAP.VP1 for K = " + K + "}\\label{table:SAP_VP1_K" + K + "_" + dictType + "}\n{\\footnotesize\n\\begin{tabular}{|@{}c@{}|";

  // Property SAP_SC1
  var table_headerSAP_SC1 = "\\begin{table}\n\\center\n\\caption{Property SAP.SC1 for K = " + K + "}\\label{table:SAP_SC1_K" + K + "_" + dictType + "}\n{\\footnotesize\n\\begin{tabular}{|@{}c@{}|";

  // Property SAP_VC1
  var table_headerSAP_VC1 = "\\begin{table}\n\\center\n\\caption{Property SAP.VC1 for K = " + K + "}\\label{table:SAP_VC1_K" + K + "_" + dictType + "}\n{\\footnotesize\n\\begin{tabular}{|@{}c@{}|";
  
  var table_headerSAP_VP2 = "\\begin{table}\n\\center\n\\caption{Property SAP.VP2 for K = " + K + " and time interval [" + timecut_start + ", " + timecut_end + "]}\\label{table:SAP_VP2_K" + K + "_" + timecut_start + "_" + timecut_end + "}\n{\\footnotesize\n\\begin{tabular}{|c|";

  var table_headerSAP_SC2 = "\\begin{table}\n\\center\n\\caption{Property SAP.SC2 for K = " + K + " and time interval [" + timecut_start + ", " + timecut_end + "]}\\label{table:SAP_SC2_K" + K + "_" + timecut_start + "_" + timecut_end + "}\n{\\footnotesize\n\\begin{tabular}{|c|";
...
// Property SAP_VP2
  for (var l=0; l<nStates; l++){
	...
	// old: UseStop position is hard coded: the last -K from the upper bound is due to un-needed UseStop on the last column
  	// old: for(var s=0; s<nStates*K-K+1-K; s=s+K){
  	// UseStop case is no longer hard coded
  	for(var s=0; s<nStates*K-K+1; s=s+K){
  		for (var x=s; x<s+K-1; x++) {
  			for (var y=x+1; y<s+K; y++) {
